...
def spotify_auth():
    ...
    encoded_client_credentials = base64.b64encode(
        bytes(f"{getenv('CLIENT_ID')}:{getenv('CLIENT_SECRET')}", "utf-8")
    ).decode("utf-8")
    ...
    for attempt in range(max_tries):
        try:
            response_json = requests.post(url=auth_endpoint, data=request_body, headers=request_headers, timeout=10)

            response_json.raise_for_status()
        except (ConnectTimeout, ReadTimeout, ConnectionError) as e:
            print(
                f"Connection error. Attempt {attempt + 1} of {max_tries}. "
